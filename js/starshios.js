...
async function showAllPerson(data) {
    ...
    for (const element of data) {
        ...
        let imgSrc = `https://starwars-visualguide.com/assets/img/starships/${filmId}.jpg`;
        let imgElement = document.createElement('img');
        imgElement.classList.add('d-block', 'user-select-none');
        
        let imageExists = await checkImageExists(imgSrc);
        if (!imageExists) {
            imgSrc = 'https://starwars-visualguide.com/assets/img/placeholder.jpg';
        }
...
function showDetails(data, url){
    ...
    const {name,model,starship_class,cost_in_credits,max_atmosphering_speed,cargo_capacity} = data
    title.textContent = name
    li[0].textContent = model
    li[1].textContent = starship_class
    li[2].textContent = cost_in_credits
    li[3].textContent = max_atmosphering_speed
    li[4].textContent = cargo_capacity
    img.src = url
}
